...
export function CatalogWorkspace() {
  ...
  const saveForm = (values: { name: string; description: string; category: string; newCategory: string; price: string; stock: string }) => {
    ...
    if (existing) { updateProduct(product); setProducts((current) => current.map((item) => item.id === product.id ? product : item)); logAuditEvent({ actor: "agent", action: "Product updated in catalog", category: "catalog", status: "success", description: `${product.name} (${product.category}) updated — price: ₹${product.price.toLocaleString("en-IN")}, stock: ${product.stock}`, referenceId: product.id, amount: product.price, currency: "INR" }); } else { addProduct(product); setProducts((current) => [...current, product]); logAuditEvent({ actor: "agent", action: "Product added to catalog", category: "catalog", status: "success", description: `${product.name} (${product.category}) added — price: ₹${product.price.toLocaleString("en-IN")}, stock: ${product.stock}`, referenceId: product.id, amount: product.price, currency: "INR" }); }
    setModal(null);
  };
  const removeProduct = () => { if (!modal?.product) return; const removed = modal.product; deleteProduct(removed.id); setProducts((current) => current.filter((product) => product.id !== removed.id)); setModal(null); logAuditEvent({ actor: "agent", action: "Product deleted from catalog", category: "catalog", status: "success", description: `${removed.name} (${removed.category}) deleted from merchant catalog`, referenceId: removed.id, amount: removed.price, currency: "INR" }); };
